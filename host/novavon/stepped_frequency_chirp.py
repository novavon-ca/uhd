...
def main():
    ...
    tx_buffer, t = dc_chirp(chirp_ampl, chirp_bw, sampling_rate, chirp_duration, ret_time_samples=True)
    ...
    for freq_idx, frequency in enumerate(center_freqs):
        ...
        logger.info(f"Acquiring data at {frequency/1e9}GHz: center freq {freq_idx+1}/{num_freqs}...")
    ...
    if plot_data:
        ...
        freqs_rx = np.fft.fftfreq(len(recv_data_list[0]), d=time_vec_rx[1]-time_vec_rx[0])
        plt.figure()
